...
int main() {
    ...
    std::string frame_shader("framebuffer");
    ...
    Shader framebufferShader(shader_location + frame_shader + std::string(".vert"),
                             shader_location + frame_shader + std::string(".frag"));
    ...
    // organized)
    // -----------------------------------------------------------------------------
    unsigned int diffuseMap = loadTexture((texture_location + "container.jpg").c_str());
    unsigned int maskTexture = loadTexture((texture_location + "mask.png").c_str());
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if(effectNum==0)
        {
            lightingShader.setBool("NightVisionFlag", NVFlag);
        }
        else
        {
            lightingShader.setBool("NightVisionFlag", false);
        }
        ...
        framebufferShader.use();
        framebufferShader.setInt("effectNum", effectNum);
        //std::cout<<blurOffset<<std::endl;
        framebufferShader.setFloat("offset", blur/300.0);
        framebufferShader.setBool("NightVisionFlag", NVFlag);
        framebufferShader.setFloat("time", lastFrame);
        glBindVertexArray(quadVAO);
        framebufferShader.setBool("texFlag", true);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, framebufferTexture); 	// use the color attachment texture as the texture of the quad plane
        glDrawArrays(GL_TRIANGLES, 0, 6);
        if(NVFlag && effectNum==0)
        {
            framebufferShader.setBool("texFlag", false);
            glActiveTexture(GL_TEXTURE0);
            glBindTexture(GL_TEXTURE_2D, maskTexture); 	// use the color attachment texture as the texture of the quad plane
            glDrawArrays(GL_TRIANGLES, 0, 6);
        }
